...
// CStat/XMotivo (via RetornoSEFAZ) são do LOTE (ex: 128 "Lote de Evento
// Processado") — o resultado do evento em si (135 registrado, ou rejeição)
// vem em CStatEvento/XMotivoEvento.
...
// ou rejeição) — RetornoSEFAZ embutido é o status do LOTE.
type RetornoCartaCorrecao struct {
	RetornoSEFAZ
	NProt         string
	CStatEvento   string
	XMotivoEvento string
...
func CartaCorrecao(c *cert.Certificado, chNFe, xCorrecao, xCondUso string, nSeqEvento int, amb Ambiente) (*RetornoCartaCorrecao, error) {
	if len(xCorrecao) < 15 {
		return nil, fmt.Errorf("sefaz: xCorrecao deve ter no mínimo 15 caracteres (tem %d)", len(xCorrecao))
	}
	if len(xCorrecao) > 1000 {
		return nil, fmt.Errorf("sefaz: xCorrecao deve ter no máximo 1000 caracteres (tem %d)", len(xCorrecao))
	}
	if len(chNFe) != 44 {
		return nil, fmt.Errorf("sefaz: chNFe deve ter 44 dígitos (tem %d)", len(chNFe))
	}
	...
	eventoAssinado, err := sign.AssinarEvento([]byte(xmlEvento), c)
	if err != nil {
		return nil, fmt.Errorf("sefaz: assinar CC-e: %w", err)
	}
	...
	var result xmlResult
	if err := xml.Unmarshal(inner, &result); err != nil {
		return nil, fmt.Errorf("sefaz: parse retEnvEvento CC-e: %w", err)
	}
...
// RetornoInutilizacao é o retorno do NFeInutilizacao. cStat/xMotivo (102 =
// "Inutilização de número homologada", ou rejeição) só existem dentro de
// infInut na resposta real — RetornoSEFAZ embutido aqui NÃO é preenchido
// (não existe cStat solto em retInutNFe), por isso os campos próprios abaixo.
...
// modelo: "55" (NF-e) ou "65" (NFC-e) — entra tanto no Id quanto no <mod>.
func (cl *Cliente) inutilizarFaixa(ctx context.Context, c *cert.Certificado, cnpj, ano, modelo, serie, nNFIni, nNFFin, justificativa string) (*RetornoInutilizacao, error) {
	if len(justificativa) < 15 || len(justificativa) > 255 {
		return nil, fmt.Errorf("sefaz: justificativa deve ter entre 15 e 255 caracteres (tem %d)", len(justificativa))
	}
	...
	xmlInut := fmt.Sprintf(
		`<inutNFe versao="4.00" xmlns="http://www.portalfiscal.inf.br/nfe">`+
			...
			`</infInut>`+
			`</inutNFe>`,
		idInut, string(cl.amb), cl.cuf, ano, cnpj, modelo,
	...
	return &RetornoInutilizacao{
		...
		CStat:        result.Ret.InfInut.CStat,
		XMotivo:      result.Ret.InfInut.XMotivo,
	}, nil
}

// Inutilizar inutiliza uma faixa de números de NF-e/NFC-e (ano-calendário corrente).
// modelo: "55" (NF-e) ou "65" (NFC-e). serie: série (ex: "1"), nNFIni/nNFFin: faixa numérica (ex: "1", "10").
// justificativa: 15-255 caracteres.
func (cl *Cliente) Inutilizar(ctx context.Context, cnpj, modelo, serie, nNFIni, nNFFin, justificativa string, c *cert.Certificado) (*RetornoInutilizacao, error) {
	ano := fmt.Sprintf("%02d", time.Now().Year()%100)
	return cl.inutilizarFaixa(ctx, c, cnpj, ano, modelo, serie, nNFIni, nNFFin, justificativa)
}
...
// ano: 2 dígitos do ano-calendário (ex: "26" para 2026). modelo: "55" ou "65".
// Cada índice i representa a faixa [nIniList[i], nFinList[i]] com justificativa xJustList[i].
func InutilizarLote(c *cert.Certificado, cuf, cnpj, ano, modelo, serie string, nIniList []int, nFinList []int, xJustList []string, amb Ambiente) ([]ResultadoInutilizacao, error) {
	...
	for i := range nIniList {
		...
		ret, err := cl.inutilizarFaixa(ctx, c, cnpj, ano, modelo, serie,
